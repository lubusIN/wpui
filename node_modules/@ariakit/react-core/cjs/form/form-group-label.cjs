"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _OHQG6SR6cjs = require('../__chunks/OHQG6SR6.cjs');
require('../__chunks/K56WQU76.cjs');




var _CWY2PHPRcjs = require('../__chunks/CWY2PHPR.cjs');
require('../__chunks/5WLTMVVQ.cjs');
require('../__chunks/7WUTS26Y.cjs');


var _72I2GWXFcjs = require('../__chunks/72I2GWXF.cjs');

// src/form/form-group-label.ts
var useFormGroupLabel = _CWY2PHPRcjs.createHook.call(void 0, 
  (_a) => {
    var _b = _a, { store } = _b, props = _72I2GWXFcjs.__objRest.call(void 0, _b, ["store"]);
    props = _OHQG6SR6cjs.useGroupLabel.call(void 0, props);
    return props;
  }
);
var FormGroupLabel = _CWY2PHPRcjs.createComponent.call(void 0, 
  (props) => {
    const htmlProps = useFormGroupLabel(props);
    return _CWY2PHPRcjs.createElement.call(void 0, "div", htmlProps);
  }
);
if (process.env.NODE_ENV !== "production") {
  FormGroupLabel.displayName = "FormGroupLabel";
}



exports.FormGroupLabel = FormGroupLabel; exports.useFormGroupLabel = useFormGroupLabel;
