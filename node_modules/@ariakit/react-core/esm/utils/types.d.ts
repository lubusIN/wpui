 ...
 */
export type Children<T = any> = React.ReactNode | RenderProp<React.HTMLAttributes<T> & React.RefAttributes<T>>;
/**
...
export interface Component<O extends Options> {
    <T extends As>(props: Props<{
        as: T;
    } & Omit<O, "as">>): React.ReactElement | null;
...
export interface Hook<O extends Options> {
    <T extends As = NonNullable<O["as"]>>(props?: Props<Options<T> & Omit<O, "as">>): HTMLProps<Options<T>>;
    displayName?: string;
}
